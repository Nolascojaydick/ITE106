...
public class PhonebookSystem extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        switch (command) {
            case "Add" -> addContact(nameField.getText(), phoneField.getText());
            case "Search" -> searchContact(nameField.getText(), phoneField.getText());
            case "Delete" -> deleteContact(nameField.getText(), phoneField.getText());
            case "Select" -> selectContact(nameField.getText(), phoneField.getText());
            case "Update" -> updateContact(nameField.getText(), phoneField.getText());
        }
    }
    ...
    public void deleteContact(String name, String phone) {
        ...
        try {
            ...
            eventField.setText(found ? "Contact deleted" : "Contact does not exist");
            BufferedWriter writer = new BufferedWriter(new FileWriter(filename));
            writer.write(contacts);
            reader.close();
            writer.close();
    ...
    public void updateContact(String name, String phone) {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                String[] splitLine = line.split(":");
                String[] splitSelectedContact = selectedContact.split(":");
                if (splitLine[0].equals(splitSelectedContact[0]) && splitLine[1].equals(splitSelectedContact[1])) {
                    eventField.setText("Contact updated to " + name + ":" + phone);
                    selectedContact = name + ":" + phone;
                    continue;
                }
                contacts += line + System.lineSeparator();
